...
import static com.yunmel.extreme.util.StringPool.EMPTY;
...
public class StringUtils {
	...
	/**
	 * Replaces all occurrences of a certain pattern in a string with a replacement
	 * string. This is the fastest replace function known to author.
	 *
	 * @param s
	...
	/**
	 * Returns <code>true</code> if string {@link #containsOnlyDigits(CharSequence)
	 * contains only digits} or signs plus or minus.
	 */
	...
	/**
	 * Converts an array object to array of strings, where every element of input
	 * array is converted to a string. If input is not an array, the result will
	 * still be an array with one element.
	 */
	...
	/**
	 * Uncapitalizes a <code>String</code>, changing the first letter to lower case.
	 ...
	 * at the specified <code>fromIndex</code> and extends to the character at index
	 * <code>toIndex - 1</code>. However, index values can be negative, and then the
	 * real index will be calculated from the strings end. This allows to specify,
	 * e.g. <code>substring(1,-1)</code> to cut one character from both ends of the
	 * string. If <code>fromIndex</code> is negative and <code>toIndex</code> is 0,
	 * it will return last characters of the string. Also, this method will never
	 ...
	 * Delimiter is <b>always</b> surrounded by two strings! If there is no content
	 * between two delimiters, empty string will be returned for that token.
	 * Therefore, the length of the returned array will always be: #delimiters + 1.
	 * <p>
	 * Method is much, much faster then regexp <code>String.split()</code>, and a
	 * bit faster then <code>StringTokenizer</code>.
	 *
	 * @param src
	 *            string to split
	 * @param delimiter
	...
	/**
	 * Splits a string in several parts (tokens) that are separated by delimiter
	 * characters. Delimiter may contains any number of character and it is always
	 * surrounded by two strings.
	 *
	 * @param src
	 *            source to examine
	 * @param d
	...
	/**
	 * Finds first occurrence of a substring in the given source but within limited
	 * range [start, end). It is fastest possible code, but still original
	 * <code>String.indexOf(String, int)</code> is much faster (since it uses char[]
	 * value directly) and should be used when no range is needed.
	 *
	 * @param src
	...
	/**
	 * Finds first index of a substring in the given source string with ignored
	 * case. This seems to be the fastest way doing this, with common string length
	 * and content (of course, with no use of Boyer-Mayer type of algorithms). Other
	 * implementations are slower: getting char array first, lower casing the source
	 * string, using String.regionMatch etc.
	 *
	 * @param src
	 *            source string for examination
	 * @param subS
	...
	/**
	 * Finds last index of a substring in the given source string in specified range
	 * [end, start] See {@link #indexOf(String, String, int, int)} for details about
	 * the speed.
	 *
	 * @param src
	 *            source to examine
	...
	/**
	 * Finds last index of a character in the given source string in specified range
	 * [end, start]
	 */
	...
	/**
	 * Tests if this string starts with the specified prefix with ignored case and
	...
	/**
	 * Finds the very last index of a substring from the specified array. It returns
	 * an int[2] where int[0] represents the substring index and int[1] represents
	 * position where substring was found. Returns <code>null</code> if noting
	 * found.
	 *
	 * @param s
	 *            source string
	...
	/**
	 * Compares string with at least one from the provided array. If at least one
	 * equal string is found, returns its index. Otherwise, <code>-1</code> is
	 * returned.
	 */
	...
	/**
	 * Compares string with at least one from the provided array, ignoring case. If
	 * at least one equal string is found, it returns its index. Otherwise,
	...
	/**
	 * Checks if string starts with at least one string from the provided array. If
	 * at least one string is matched, it returns its index. Otherwise,
	 * <code>-1</code> is returned.
	 */
	...
	/**
	 * Checks if string ends with at least one string from the provided array. If at
	 * least one string is matched, it returns its index. Otherwise, <code>-1</code>
	 * is returned.
	 */
	...
	/**
	 * Returns the very first index of any char from provided string, starting from
	 * specified index offset. Returns index of founded char, or <code>-1</code> if
	 * nothing found.
	 */
	...
	/**
	 * Returns first index of a whitespace character, starting from specified index
	 * offset.
	 */
	...
	/**
	 * Strips everything from the first appearance of given char. Character IS NOT
	 * included in the returned string.
	 */
	...
	/**
	 * Returns indexes of the first string region. Region is defined by its left and
	 * right boundary. Return value is an array of the following indexes:
	 * <ul>
	 ...
	 * decreased to include one escape character. If region is not founded,
	...
	/**
	 * Converts string charset. If charset names are the same, the same string is
	 * returned.
	 */
	 ...
	 * missing, nothing will be cut and <code>null</code> is returned. If indexes of
	 * left or right strings are wrong, empty string is returned.
	 */
	...
	/**
	 * Changes CamelCase string to lower case words separated by provided separator
	 * character. The following translations are applied:
	 * <ul>
	 ...
	 * first character are replaced only by their lower case equivalent, and are not
	 * preceded by a separator (<code>theFOO</code> to <code>the_foo</code>).
	 * <li>An upper case character in the first position of the CamelCase name is
	 * not preceded by a separator character, and is translated only to its lower
	 * case equivalent. (<code>Foo</code> to <code>foo</code> and not
	 * <code>_foo</code>)
	 * <li>An upper case character in the CamelCase name that is already preceded by
	 * a separator character is translated only to its lower case equivalent, and is
	 * not preceded by an additional separator. (<code>user_Name</code> to
	 * <code>user_name</code> and not <code>user__name</code>.
	 * </ol>
	 * <li>If the CamelCase name starts with a separator, then that separator is not
	 * included in the translated name, unless the CamelCase name is just one
	 * character in length, i.e., it is the separator character. This applies only
	 * to the first character of the CamelCase name.
	 * </ul>
	 */
	...
	/**
	 * Finds common prefix for several strings. Returns an empty string if arguments
	 ...
	 *            the string to convert
	 * @param locale
	 *            apply this locale's rules, if <code>null</code> default locale is
	 *            used
	 * @return the string, converted to lower case, or <code>null</code> if the
	 *         string is <code>null</code>
	 */
